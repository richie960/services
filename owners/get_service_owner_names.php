  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      ...
      function fetchServiceOwners(query = '') {
        fetch(`get_service_owners.php?query=${encodeURIComponent(query)}`)
        ...
        fetch(`get_reviews.php?owner_id=${ownerId}`)
          ...
          .then(reviewsData => {
            ...
            reviewsData.forEach(review => {
              ...
              reviewDiv.innerHTML = `<p><strong>${review.author}</strong>: ${review.content}</p>`;
      ...
      function setupAutocomplete() {
        fetch('get_service_owner_names.php')
          ...
          .catch(error => {
            console.error('Error fetching service owner names:', error);
          });
      }

      searchInput.addEventListener('input', function() {
        fetchServiceOwners(this.value);
      });

      bookingButton.addEventListener('click', function() {
        const ownerId = modalDetails.dataset.ownerId;
